...
$(document).ready(function() {
	...
	$("#but_done").click(function() {
		...
		if(isDirectSubmit) {
			...
			// Show saved data in table for Review
...
validate = function(clarityJSON) {
	...
	//alert(clarityJSON.timesheet);
	/*if(clarityJSON.timesheet != undefined) {
		isValid = true;
	}
	else {
		isValid = false;
...
getClarityJSON = function() {
	...
	try {
		...
		clarityJSON.timesheet.id = $('#timesheetId').val();	
		...
		clarityJSON.timesheet.status1 = $('#timesheet #id').val();
		...
		var weekStrtDate = datesArray[1].replace("\r","").concat("/11"); //TODO: Remove hardcoding of year
		...
		var today = new Date();
		var todayMMddYY = (today.getMonth()+1) + "/" + today.getDate() + "/" + (today.getFullYear()+"").substring(2,4);
		clarityJSON.timesheet.lastModifDate = todayMMddYY;
		...
		alert('Invalid clarity data:\n debugInfo:' + e.description)
	}
	//return clarityJSON; 
}
